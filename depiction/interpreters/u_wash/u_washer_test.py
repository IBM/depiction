...
class UWasherTestCase(unittest.TestCase):
    def testConstructor(self):
        ...
        def test_routine(explainer_key, explainer_cls, model):
            with mock.patch('depiction.interpreters.u_wash.u_washer.{}.__init__'.format(explainer_cls),
                side_effect = dummy_init) as mock_constructor:
    ...
    def testInterpret(self):
        ...
        interpreter = UWasher('lime', model, **{'training_data': np.array([[0,0],[1,1]])})
        test_config = {'dummy_config': 10}
        test_callback_kwargs = {}
        dummy_sample = [10, 15]

        with mock.patch.object(interpreter.explainer, 'explain_instance',
            side_effect=dummy_interpret) as mock_explain:
            explanation = interpreter.interpret(dummy_sample, test_callback_kwargs, test_config, path=None)
        ...
        with mock.patch.object(interpreter.explainer, 'explain_instance',
            side_effect=dummy_interpret) as mock_explain:
            dummy_path = 'tests'
            explanation = interpreter.interpret(dummy_sample, test_callback_kwargs, test_config, path=dummy_path)
            mock_explain.assert_called_once()
