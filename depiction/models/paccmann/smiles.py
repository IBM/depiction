...
MAXIMUM_NUMBER_OF_RINGS = int(os.environ.get(
    'PACCMANN_MAXIMUM_NUMBER_OF_RINGS',
    9
))
...
ATOM_MAPPING = {
    '2': 1, '7': 2, 'O': 3, '[O]': 4, '#': 5, '(': 6, 'P': 7,
    'Cl': 8, 'C': 9, 'N': 10, 'Br': 11, 'F': 12, ')': 13, '=': 14,
    '9': 15, '4': 16, '1': 17, '6': 18, 'I': 19, '[N+]': 20,
    '[NH]': 21, '.': 22, 'S': 23, '[O-]': 24, '3': 25, '8': 26,
    '5': 27, PADDING_ATOM: 0
}
REVERSED_ATOM_MAPPING = {
    index: atom
    for atom, index in ATOM_MAPPING.items()
}
CMAP = cm.Oranges
COLOR_NORMALIZERS = {
...
def smiles_attention_to_svg(
    ...
    logger.debug('SMILES attention range: [{},{}]'.format(
        min(smiles_attention), max(smiles_attention)
    ))
    atoms, smiles_attention = remove_padding_from_atoms_and_smiles_attention(
        atoms, smiles_attention
    )
    logger.debug('atoms and SMILES after removal:\n{}\n{}'.format(
        atoms, smiles_attention
    ))
    logger.debug('SMILES attention after padding removal:\n{}'.format(
        smiles_attention
    ))
    logger.debug(
    ...
    normalize = COLOR_NORMALIZERS.get(
        ...
        vmin=min(smiles_attention),
        vmax=2*max(smiles_attention)
    )
